...
<task type="auto">
  ...
  <action>
    **1. Add `coreOverride` to ExtensionBladeConfig in `src/extensions/ExtensionAPI.ts`:**
    ...
    Also widen the `title` field in `ExtensionBladeConfig` from `title: string;` to `title: string | ((props: any) => string);` so extensions can pass dynamic title functions (needed by content-viewers extension for filename-based titles).
