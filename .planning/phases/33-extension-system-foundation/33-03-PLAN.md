...
must_haves:
  truths:
    ...
    - "Extensions are discovered and activated when a repository is opened, and deactivated when it is closed"
    - "When the user switches repositories, extensions from the old repo are deactivated and extensions from the new repo are discovered and activated"
  artifacts:
    ...
    - path: "src/extensions/index.ts"
      ...
      contains: "export"
    - path: "src/App.tsx"
      provides: "Extension lifecycle wired into app initialization"
      contains: "useExtensionHost"
  key_links:
    ...
    - from: "src/App.tsx"
      ...
      via: "useEffect triggers discoverExtensions and activateAll on repo open"
      pattern: "discoverExtensions|activateAll|deactivateAll"
---

<objective>
Build the ExtensionHost Zustand store and per-extension ExtensionAPI facade that manages the full extension lifecycle: discovery, apiVersion validation, activation via dynamic import, registration tracking, and clean deactivation. Wire the extension lifecycle into the application startup so extensions are discovered and activated when a repository is opened.
...
Output: Complete extension system runtime in `src/extensions/` -- ready for extensions to be loaded, automatically triggered on repository open/close.
</objective>
...
**discoverExtensions(repoPath: string):**
1. Set `isDiscovering: true`.
2. Construct the extensions directory: `const extensionsDir = repoPath + "/.flowforge/extensions"` (the `repoPath` argument is the absolute repo root from `repoStatus.repoPath`).
3. Call the Rust command: `const manifests = await commands.discoverExtensions(extensionsDir)`.
4. For each manifest, check `manifest.apiVersion !== CURRENT_API_VERSION`. If incompatible, set status to `"error"` with an error message, and call `toast.error(...)` with a user-visible message.
5. Compatible extensions get status `"discovered"`.
6. Update state with the new extensions Map (copy-on-write: `new Map(extensions)`).
7. Set `isDiscovering: false`.
...
<task type="auto">
  <name>Task 4: Wire ExtensionHost into app initialization lifecycle</name>
  <files>
    src/App.tsx
  </files>
  <action>
Wire the extension system into App.tsx so that extensions are automatically discovered and activated when a repository is opened, and deactivated when the repository is closed or changed.

**Pattern to follow:** App.tsx already uses `const status = useRepositoryStore((s) => s.repoStatus)` which is `RepoStatus | null`. When a repo is open, `status` is truthy and `status.repoPath` gives the absolute path. When no repo is open, `status` is null. The existing code already conditionally renders based on `status` (line 86: `{status ? <RepositoryView /> : <WelcomeView />}`).
...
2. Inside the `App` function, select the extension host actions:
   ```typescript
...
3. Add a new `useEffect` that reacts to `status` changes. Place it after the existing "Initialize theme, settings..." useEffect block but before the "Listen for file watcher events" useEffect. The effect should:
   ...
   **Key behaviors:**
   - When `status` transitions from null to a RepoStatus object (repo opened): calls `discoverExtensions(repoPath)` then `activateAll()`.
   - When `status` transitions from a RepoStatus to null (repo closed): calls `deactivateAll()`.
   - When `status` changes to a *different* RepoStatus (repo switch): the cleanup function runs `deactivateAll()` first (tearing down old extensions), then the effect body runs `discoverExtensions` + `activateAll` for the new repo.
   - The `status` object reference changes when the repo changes (different `repoPath`), so React will re-run the effect on repo switch.

4. Do NOT modify any other existing code in App.tsx. Only add the import, the three selector lines, and the one useEffect.

**What NOT to do:**
- Do NOT add a separate hook file for this -- keep it in App.tsx following the existing pattern where all init-on-mount effects live.
- Do NOT use `useRepositoryStore.subscribe()` -- the useEffect + dependency array approach is the established pattern in this codebase.
- Do NOT call `discoverExtensions` without a repo path -- always guard behind `if (status)`.
  </action>
  <verify>
1. `npx tsc --noEmit` -- no new errors (the import resolves, the hook calls type-check).
2. `npx vitest run` -- all existing tests pass (no behavioral changes to existing functionality).
3. Read App.tsx and confirm: the useEffect is present, reacts to `status`, calls `discoverExtensions(status.repoPath)` then `activateAll()` when status is truthy, calls `deactivateAll()` when status is falsy, and the cleanup function calls `deactivateAll()`.
  </verify>
  <done>
App.tsx wires extension lifecycle to repository state: opening a repo triggers discoverExtensions + activateAll, closing a repo triggers deactivateAll, switching repos deactivates old extensions then discovers and activates new ones. The integration follows the existing useEffect + selector pattern used by all other App.tsx initialization code.
  </done>
</task>
...
9. App.tsx contains useEffect that calls discoverExtensions + activateAll when repoStatus is truthy
10. App.tsx contains useEffect cleanup that calls deactivateAll on repo close or repo switch
</verification>
...
- discoverExtensions accepts repoPath and calls Rust command with {repoPath}/.flowforge/extensions
...
- App.tsx wires extension lifecycle: discover+activate on repo open, deactivate on repo close/switch
