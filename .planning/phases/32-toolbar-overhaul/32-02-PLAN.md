...
Output: 6 new toolbar component files, refactored Header.tsx, updated SyncButtons, App.tsx import.
</objective>
...
- Add `<SyncProgressDisplay>` as a separate element between structural components and toolbar (or at a suitable position), rendered only when there's active sync progress. The SyncButtons component will continue to own the sync progress state -- extract SyncProgressDisplay rendering from SyncButtons if needed, or keep it inside a headless SyncController.

The header layout should be:
```
[Logo] [RepoSwitcher] [BranchSwitcher] [ProcessNav] | [SyncProgress?] | [Toolbar with overflow]
```
...
**2. Refactor SyncButtons.tsx:**

The SyncButtons component currently renders its own buttons. After the toolbar migration, it should become a "headless" sync controller:
- Keep all the React Query mutation logic (push/pull/fetch mutations with progress channels).
- Keep the `progress` state and `SyncProgressDisplay`.
- Remove the button JSX -- the buttons are now toolbar actions.
- Export the sync state (mutations, progress) so the toolbar action `isLoading` functions can read it.

**Simplest approach:** Convert SyncButtons to a headless `<SyncController>` component that:
- Renders only `{progress && <SyncProgressDisplay progress={progress} />}`.
- Exposes mutation functions via a Zustand store or module-level object that toolbar actions can call.
- Actually, the toolbar actions in `toolbar-actions.ts` already have their own execute functions that call Tauri commands directly. So SyncButtons/SyncController just needs to provide progress display and react-query cache invalidation.

**Revised simplest approach:** Since the toolbar actions already handle sync execution directly (from Plan 01 Task 2), SyncButtons can be simplified to just render `<SyncProgressDisplay>` when there's progress. OR remove SyncButtons entirely and handle progress at the Toolbar level.

**Final approach:** Remove the SyncButtons component entirely. The toolbar actions handle sync execution. The progress display is handled via toasts (which the toolbar actions already send via `toast.success`/`toast.error`). The `SyncProgressDisplay` component with its detailed transfer percentage can be preserved as a visual element rendered directly in `Header.tsx` if the user wants inline progress, but for now the toast-based progress from the toolbar actions is sufficient.

If the existing SyncButtons had important progress tracking that must be preserved:
- Keep `SyncButtons.tsx` as `SyncController.tsx`, but make it a headless component that only renders `<SyncProgressDisplay>` and provides mutation state.
- Place `<SyncController />` in Header.tsx.
- The toolbar action execute functions should trigger the mutations from this controller (e.g., via custom events or a shared store).

Given the complexity, the pragmatic approach: Keep SyncButtons.tsx working for now but hidden. The toolbar renders its own fetch/pull/push buttons that call Tauri commands directly with toast feedback. In Header.tsx, do NOT render `<SyncButtons />` anymore. The detailed progress display (`SyncProgressDisplay`) is a nice-to-have that can be re-added in a future polish pass.

**3. Wire toolbar-actions.ts import in App.tsx:**

Add `import "../commands/toolbar-actions";` (or the correct relative path) near the top of `src/App.tsx` as a side-effect import. This ensures all core toolbar actions are registered before the first render. Place it alongside any existing side-effect imports (e.g., blade registrations).

Check if there's an existing pattern for side-effect imports in App.tsx or main.tsx. If blade registrations use a different pattern (e.g., imported from a barrel), follow that pattern.
  </action>
  <verify>
...
6. App.tsx imports toolbar-actions.ts.
  ...
  <done>
Header.tsx is refactored to a thin shell (~80-100 lines) that composes [Logo, RepoSwitcher, BranchSwitcher, ProcessNav, Toolbar]. All toolbar actions render from the registry via the Toolbar component. SyncButtons is either removed or converted to headless. toolbar-actions.ts is imported in App.tsx for side-effect registration. Stash-and-switch dialog remains functional.
  </done>
</task>
