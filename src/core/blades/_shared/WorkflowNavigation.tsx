...
export function WorkflowNavigation({ className }: WorkflowNavigationProps) {
  ...
  const visibleWorkflows = useMemo(
    () =>
      allWorkflows.filter((w) => {
        ...
        return blades.has(w.rootBlade.type as string) || (w.fallbackBlade && blades.has(w.fallbackBlade.type as string));
      }),
    [blades, allWorkflows],
  );
  ...
  useEffect(() => {
    ...
    if (active && !blades.has(active.rootBlade.type as string) && !active.fallbackBlade) {
