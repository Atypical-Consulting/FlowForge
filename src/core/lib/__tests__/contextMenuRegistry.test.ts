...
describe("ContextMenuRegistry", () => {
  ...
  it("registers and retrieves item by location", () => {
    const { register, getItemsForLocation } = useContextMenuRegistry.getState();
  ...
  it("separates items by group alphabetically", () => {
    const { register, getItemsForLocation } = useContextMenuRegistry.getState();
    register(makeItem({ id: "z-item", location: "file-tree", group: "zebra" }));
    register(makeItem({ id: "a-item", location: "file-tree", group: "alpha" }));
  ...
  it("unregisterBySource removes all items for source", () => {
    const { register, unregisterBySource } = useContextMenuRegistry.getState();
  ...
  it("hideMenu clears activeMenu", () => {
    const { register, showMenu, hideMenu } = useContextMenuRegistry.getState();
    register(makeItem({ id: "action", location: "file-tree" }));
