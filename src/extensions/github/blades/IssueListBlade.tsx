/**
 * Issue list blade with state filter tabs and infinite scroll.
 *
 * Displays a paginated list of issues for the selected GitHub remote.
...
export function IssueListBlade() {
  ...
  if (!remote) {
    return (
      <BladeContentEmpty
        ...
        detail="Open a repository with a GitHub remote to view issues."
      />
    );
  }
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Filter tabs */}
      <div className="px-3 py-2" role="tablist" aria-label="Issue state filter">
      ...
      <div className="flex-1 min-h-0">
        <IssueList
          ...
          onSelect={(issue) =>
            openBlade(
              "ext:github:issue",
              { owner: remote.owner, repo: remote.repo, number: issue.number },
...
function IssueList({ owner, repo, state, onSelect }: IssueListInnerProps) {
  const query = useIssueList(owner, repo, state);
        ...
        >
          <div className="flex items-start gap-2">
            <div className="pt-0.5 shrink-0">
              <StatusBadge state={issue.state} type="issue" />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm text-ctp-text truncate">{issue.title}</p>
              <div className="flex items-center gap-2 mt-1 text-xs text-ctp-overlay0">
                <span className="text-ctp-subtext0">#{issue.number}</span>
