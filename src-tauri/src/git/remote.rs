...
pub fn pull_branch(
    ...
    if analysis.is_fast_forward() {
        ...
        // work. The checkout baseline is the current HEAD tree, so the branch
        // ref must only be advanced once the checkout succeeded; moving the
        // ref first would make the old content look like local edits that a
        // safe checkout leaves in place (index and worktree stay stale).
...
mod tests {
    ...
    #[test]
    fn pull_fast_forward_updates_worktree_and_index() {
        let fx = fixture();
        let other = other_clone(&fx);
        commit_file(&other, "a.txt", "a2");
        commit_file(&other, "new.txt", "n");
        push_branch(&other, "origin", noop_sink()).unwrap();

        pull_branch(&fx.work, "origin", noop_sink()).unwrap();

        let workdir = fx.work.workdir().unwrap();
        assert_eq!(fs::read_to_string(workdir.join("a.txt")).unwrap(), "a2");
        assert_eq!(fs::read_to_string(workdir.join("new.txt")).unwrap(), "n");
        // Advancing the ref before the checkout would leave the old content
        // in place and show every incoming change as staged; status must be
        // clean after a fast-forward of a clean tree.
        let statuses = fx.work.statuses(None).unwrap();
        let dirty: Vec<String> = statuses
            .iter()
            .map(|s| s.path().unwrap().to_string())
            .collect();
        assert!(dirty.is_empty(), "unexpected status entries: {:?}", dirty);
    }
}
